...
public sealed class ClassificationsApiTests
{
    ...
    [Fact] public async Task GetMissingReturnsNotFound(){using var f=CreateFactory();var r=await f.CreateClient().GetAsync("/api/v1/classifications/missing");await AssertProblemJsonAsync(r,HttpStatusCode.NotFound);}
    [Fact] public async Task NotFoundUsesProblemJson(){using var f=CreateFactory();var r=await f.CreateClient().GetAsync("/api/v1/classifications/missing");await AssertProblemJsonAsync(r,HttpStatusCode.NotFound);}
    [Fact] public async Task NotFoundIncludesProblemFields(){using var f=CreateFactory();var r=await f.CreateClient().GetAsync("/api/v1/classifications/missing");await AssertProblemJsonAsync(r,HttpStatusCode.NotFound);}
    [Fact] public async Task InvalidPostReturnsBadRequest(){using var f=CreateFactory();var r=await f.CreateClient().PostAsJsonAsync("/api/v1/classifications",ValidCreate() with { FiscalYear=2023 },JsonOptions);await AssertProblemJsonAsync(r,HttpStatusCode.BadRequest,expectErrors:true);}
    [Fact] public async Task InvalidPostContainsErrors(){using var f=CreateFactory();var r=await f.CreateClient().PostAsJsonAsync("/api/v1/classifications",ValidCreate() with { FiscalYear=2023 },JsonOptions);await AssertProblemJsonAsync(r,HttpStatusCode.BadRequest,expectErrors:true);}
    ...
    [Fact] public async Task DuplicatePostReturnsConflict(){using var f=CreateFactory();var r=await f.CreateClient().PostAsJsonAsync("/api/v1/classifications",ValidCreate() with { FiscalYear=2024,Market="Acciones",Instrument="NUAM-A",PaymentDate=new DateOnly(2024,1,15),EventSequence="EVT-ACC-001" },JsonOptions);await AssertProblemJsonAsync(r,HttpStatusCode.Conflict);}
    ...
    [Fact] public async Task PutInvalidIfMatchReturnsBadRequest(){using var f=CreateFactory();var req=new HttpRequestMessage(HttpMethod.Put,"/api/v1/classifications/CAL-2024-001"){Content=JsonContent.Create(ValidUpdate(),options:JsonOptions)};req.Headers.TryAddWithoutValidation("If-Match","not-base64");var r=await f.CreateClient().SendAsync(req);await AssertProblemJsonAsync(r,HttpStatusCode.BadRequest,expectErrors:true);}
    [Fact] public async Task PutMismatchedIfMatchReturnsConflict(){using var f=CreateFactory();var req=new HttpRequestMessage(HttpMethod.Put,"/api/v1/classifications/CAL-2024-001"){Content=JsonContent.Create(ValidUpdate(),options:JsonOptions)};req.Headers.TryAddWithoutValidation("If-Match","AQIDBA==");var r=await f.CreateClient().SendAsync(req);await AssertProblemJsonAsync(r,HttpStatusCode.Conflict);}
    ...
    [Fact] public async Task DeleteMissingReturnsNotFound(){using var f=CreateFactory();var r=await f.CreateClient().DeleteAsync("/api/v1/classifications/missing");await AssertProblemJsonAsync(r,HttpStatusCode.NotFound);}
    ...
    private static async Task AssertProblemJsonAsync(HttpResponseMessage response, HttpStatusCode expectedStatus, bool expectErrors=false)
    {
        ...
        if(expectErrors)Assert.True(document.RootElement.TryGetProperty("errors",out _));
    }
