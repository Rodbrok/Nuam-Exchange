...
public sealed class JwtTokenService(IOptions<JwtOptions> options, TimeProvider timeProvider) : IJwtTokenService
{
    ...
    private static void ValidateRequest(JwtTokenRequest request)
    {
        ...
        {
            throw new ArgumentException("JWT request must contain a user id.", nameof(request));
        }

        if (string.IsNullOrWhiteSpace(request.Email))
        {
            throw new ArgumentException("JWT request must contain an email.", nameof(request));
        }
